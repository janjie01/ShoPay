...
const corsOptions = {
  origin: ["https://shopayapp.netlify.app"],
...
app.use((req, res, next) => {
  res.header('Access-Control-Allow-Origin', 'https://shopayapp.netlify.app'); // Replace * with your frontend's URL in production
...
app.post("/register", (req, res) => {
  ...
  const insertUserQuery =
    "INSERT INTO users (`name`, `username`, `birthdate`, `address`, `role`, `email`, `profile_pic`, `password`) VALUES (?)";
  ...
  db.query(checkEmailQuery, [req.body.email], (errEmail, resultEmail) => {
    ...
    db.query(
      ...
      (errUsername, resultUsername) => {
        ...
        bcryptjs.hash(req.body.password, 10, (hashErr, hashedPassword) => {
          if (hashErr) {
...
app.get("/profile", authenticateToken, (req, res) => {
  ...
  const getPurchaseQuery =
    "SELECT product_name, quantity, purchased_date FROM purchase WHERE user_id = ?";

  db.query(getUserQuery, [userId], (userErr, userResult) => {
    if (userErr) {
      console.error(
        "Error executing MySQL query for user information:",
        userErr
      );
    ...
    db.query(getPurchaseQuery, [userId], (purchaseErr, purchaseResult) => {
      if (purchaseErr) {
        console.error(
          "Error executing MySQL query for purchased items:",
          purchaseErr
        );
        res.status(500).json({ error: "Internal Server Error" });
        return;
...
app.post("/add_product", async (req, res) => {
  ...
  const insertProductQuery =
    "INSERT INTO product (`product_name`, `product_description`, `product_photo`, `product_price`, `product_qty`) VALUES (?)";
  ...
  try {
    ...
    db.query(
      checkProductNameQuery,
      [req.body.product_name],
      (errProductName, resultProductName) => {
        if (errProductName) {
...
app.put("/update/:tableName/:id", async (req, res) => {
  ...
  if (tableName === "product") {
    updateQuery =
      "UPDATE product SET product_name=?, product_description=?, product_price=?, product_qty=? WHERE product_id=?";
    values = [
      req.body.product_name,
      req.body.product_description,
      req.body.product_price,
    ...
    updateQuery =
      "UPDATE users SET name=?, username=?, birthdate=?, address=?, role=?, email=? WHERE user_id=?";
    values = [
      req.body.name,
      req.body.username,
      req.body.birthdate,
  ...
  try {
    ...
    db.query(updateQuery, values, (updateErr, updateResult) => {
      if (updateErr) {
        console.error(
          `Error updating record in ${tableName} table:`,
          updateErr
        );
        return res.status(500).json({ Error: "Internal Server Error" });
      }
      ...
      return res
        .status(200)
        .json({ Status: `${tableName} record updated successfully` });
    });
  } catch (error) {
    console.error(
      `Error updating record in ${tableName} table:`,
...
app.post("/add-to-cart", authenticateToken, (req, res) => {
  ...
  db.query(checkCartQuery, [userId, productId], (checkErr, checkResult) => {
    ...
    const addToCartQuery =
      "INSERT INTO cart (user_id, product_ids, quantity) VALUES (?, ?, 1)";
    db.query(addToCartQuery, [userId, productId], (addErr, addResult) => {
      if (addErr) {
        console.error("Error adding to cart:", addErr);
        return res
          .status(500)
          .json({ error: "Internal Server Error - Add to Cart" });
      }
...
app.post("/checkout", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "INSERT INTO purchase (user_id, product_name, quantity) VALUES (?, ?, ?)",
      [userId, productName, 1]
    );

    // Update Product table to reduce quantity
    await db.query(
      "UPDATE product SET product_qty = product_qty - 1 WHERE product_name = ?",
      [productName]
    );
